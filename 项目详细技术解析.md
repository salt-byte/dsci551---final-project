...
def find(self, query=None):
    ...
    def match(doc, query):  # 内部函数：检查文档是否匹配查询
        for key, value in query.items():
            ...
            # Exact match first (优先级最高)
            if cur == value:
                continue
            # Case-insensitive match for strings (字符串大小写不敏感)
            if isinstance(cur, str) and isinstance(value, str):
                if cur.lower() == value.lower():
                    continue
            # Type conversion for numbers (数字类型转换)
            ...
            if isinstance(value, bool) and isinstance(cur, str):
                ...
                if bool_str.lower() == cur.lower():
                    continue
            # None/null matching (None/null 匹配)
...
**作用**：根据查询条件过滤文档，类似 SQL 的 `WHERE` 子句，支持智能类型匹配
...
**问题背景**：
- 在实际应用中，查询值可能是字符串类型（如从 UI 输入框获取）
- 但 JSON 数据中的值可能是数字、布尔值、None 等不同类型
- 简单的严格相等比较会导致类型不匹配，找不到数据

**解决方案**：实现了**多层级智能匹配策略**，按优先级依次尝试：
...
**match() 内部函数的完整流程**：
1. 遍历查询条件的每个键值对
2. 从文档中提取对应字段的值（支持嵌套字段）
3. 按优先级依次尝试上述 5 种匹配策略
4. 如果所有策略都失败，文档不匹配
5. 只有所有条件都满足，才返回 `True`
...
# 单个条件 - 字符串匹配
coll.find({"name": "john"})  # 大小写不敏感
# 结果：[{"name": "John", ...}, {"name": "John", ...}]

# 数字类型转换
coll.find({"age": "30"})  # 字符串 "30" 匹配整数 30
# 结果：[{"name": "John", "age": 30, ...}, {"name": "John", "age": 30, ...}]

# 布尔值匹配
coll.find({"verified": "true"})  # 字符串 "true" 匹配布尔值 True
# 结果：[{"name": "John", "verified": True, ...}]

# None 匹配
coll.find({"verified": "none"})  # 字符串 "none" 匹配 None
# 结果：[{"name": "John", "verified": None, ...}]

# 多个条件（AND 关系）
coll.find({"name": "John", "age": "30"})  # 混合类型匹配
# 结果：[{"name": "John", "age": 30, ...}, {"name": "John", "age": 30, ...}]
...
coll = Collection(data)
coll.find({"user.name": "john"})  # 点号表示法 + 大小写不敏感
...
| 优先级 | 匹配策略 | 适用场景 | 示例 |
|--------|---------|---------|------|
| 1 | 精确匹配 | 类型和值完全相等 | `123 == 123` |
| 2 | 大小写不敏感 | 两个字符串 | `"NYC"` 匹配 `"nyc"` |
| 3 | 数字类型转换 | 数字 ↔ 字符串 | `"123"` 匹配 `123` |
| 4 | 布尔值匹配 | 布尔值 ↔ 字符串 | `"true"` 匹配 `True` |
| 5 | None/null 匹配 | None ↔ 字符串 | `"none"` 匹配 `None` |
...
**作用**：将整个 JSON/JSONL 文件一次性加载到内存中

**为什么需要**：
- **Notebook 场景**：只需要 `load_json_chunks()` 分块处理大文件
- **Streamlit 应用场景**：需要完整数据在内存中，供用户实时查询和操作

**实现原理**：
1. 调用 `load_json_chunks()` 分块读取文件
2. 使用 `extend()` 将所有块合并成一个完整列表
3. 返回包含所有数据的列表

**与 load_json_chunks() 的区别**：

| 特性 | `load_json_chunks()` | `load_json_file()` |
|------|---------------------|-------------------|
| 返回类型 | 生成器（Generator） | 列表（List） |
| 内存使用 | 低（逐块处理） | 高（全部加载） |
| 适用场景 | 大文件分块处理 | 中小文件完整加载 |
| 数据访问 | 只能顺序访问块 | 可以随机访问所有数据 |
| Notebook | ✅ 已定义 | ❌ 未定义（Streamlit 新增） |

**使用场景**：
- ✅ Streamlit 应用中加载文件供用户查询
- ✅ 需要完整数据在内存中进行实时操作
- ❌ 超大文件（GB 级别，可能导致内存溢出）
...
results = collection.find({"name": "John"})  # 需要完整数据
```

---

### 5.4 get_all_fields() - 字段提取器
...
**作用**：从 JSON 数据结构中递归提取所有字段路径（支持嵌套字段）

**详细流程**：

1. **处理列表**：如果数据是列表，使用第一个元素作为样本

2. **递归遍历字典**：
   - 提取每个键作为字段路径
   - 如果值是嵌套字典或列表，递归提取

3. **点号表示法**：嵌套字段使用点号连接，如 `"user.name"`

4. **去重和排序**：使用 `set` 去重，最后排序返回
